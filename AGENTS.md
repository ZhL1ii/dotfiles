...
- 新增小型 helper 函数时，即使函数很短，也应说明它服务的配置场景、关键参数含义和返回/副作用。例如复制路径类函数要说明复制对象、路径格式参数、通知文案参数和剪贴板写入行为。
- 对插件 command、callback、mapping handler 这类由插件注入上下文的函数，应说明注入参数来自哪里、代表什么对象，以及为什么不能直接使用当前 buffer/window 的普通状态。
- 关键局部变量如果来自插件对象、Neovim 特殊展开、外部命令输出或配置查找，应说明数据来源和后续用途。例如 `node.path or node:get_id()` 这类表达式要说明首选字段、fallback 字段和兼容目的。
- 包装函数或快捷键映射只是在传不同参数时，也要说明参数差异对应的用户场景，避免只留下多个看起来相同的匿名函数。
